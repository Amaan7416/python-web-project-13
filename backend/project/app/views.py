    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                Modified_Date = None
                if not pd.isna(row['Modified_Date']):
                ...
                # Terminal me successful ID show karega
                print("Uploaded CSV ID:", row['ID'])

            # --------------------------------
            # AGAR KISI EK ROW ME ERROR HO
            # --------------------------------
            except Exception as e:
                print("ERROR IN ROW:", index + 2)
                print("CSV ID:", row['ID'])
                print("ERROR:", e)
                # Is row ko skip karke next row par jayega
                continue

        # Saari rows process hone ke baad
        return HttpResponse(
            "File data is uploaded or updated"
        )
        ...
        serialized_data = [
            {
                ...
                'Reviewer_Comments': conn.Reviewer_Comment,
            }
    ...
    if request.method=='GET':
        try:
            ...
            connection_data = {
                ...
                "Reviewer_Comments": connection.Reviewer_Comment,
            }
